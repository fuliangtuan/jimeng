...
export type RegionKey = "CN" | "US" | "HK" | "JP" | "SG" | "PH";
...
export function resolveResolution(
  ...
  if (userModel === "nanobanana") {
    if (regionKey === "US") {
      // US 站: 允许 1k/2k 分辨率和自定义比例
      // 不做强制处理，回退到默认查找逻辑
    } else if (regionKey === "HK" || regionKey === "JP" || regionKey === "SG" || regionKey === "PH") {
      // HK/JP/SG/PH 站: 强制 1k 分辨率，但 ratio 可自定义
...
export function getBenefitCount(
  ...
  if (regionKey === "HK" || regionKey === "JP" || regionKey === "SG" || regionKey === "PH") {
