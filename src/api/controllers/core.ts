...
export function parseRegionFromToken(refreshToken: string): RegionInfo {
  ...
  const isPH = token.startsWith('ph-');
  const isInternational = isUS || isHK || isJP || isSG || isPH;
...
export function getAssistantId(regionInfo: RegionInfo): number {
  ...
  if (regionInfo.isPH) return DEFAULT_ASSISTANT_ID_PH;
...
export function generateCookie(refreshToken: string) {
  const { isUS, isHK, isJP, isSG, isPH } = parseRegionFromToken(refreshToken);
  const token = (isUS || isHK || isJP || isSG || isPH) ? refreshToken.substring(3) : refreshToken;
  ...
  else if (isPH) storeRegion = 'hk'; // PH likely uses HK store region as well, based on SG pattern (though user header said ph, cookie might group)
...
export async function request(
  ...
  if (isUS) {
    ...
    region = REGION_US;
  } else if (isHK || isJP || isSG || isPH) {
    // HK, JP, SG and PH regions use the same SG base URL
    if (uri.startsWith("/commerce/")) {
