...
export function getModel(model: string, regionInfo: RegionInfo) {
  ...
  } else if (regionInfo.isHK || regionInfo.isJP || regionInfo.isSG || regionInfo.isPH) {
...
export async function generateVideo(
  ...
  // 只有 video-3.0, video-3.0-fast 和 3.5-pro 支持 resolution 参数
  // (之前的注释说3.0-pro和3.5-pro不支持，但最新的curl显示3.5-pro是支持的)
  const supportsResolution = 
    (model.includes("vgfm_3.0") || model.includes("vgfm_3.0_fast") || model.includes("vgfm_3.5_pro")) 
    && !model.includes("vgfm_3.0_pro"); // 明确排除3.0-pro，保留3.0, 3.0-fast, 3.5-pro
